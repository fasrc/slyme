...
class x_scontrol_JobID_to_raw_scontrol_text(lazydict.Extension):
	...
	def __call__(self, JobID):
		try:
			_raw_scontrol_text = _yield_raw_scontrol_text_per_job(jobs=[JobID]).next()
...
class x_scontrol_raw_scontrol_text_to_jobatts(lazydict.Extension):
	...
	def __call__(self, _raw_scontrol_text):
		...
		return [ d[k] for k in self.target ]
...
class x_sacct_raw_sacct_text_to_jobatts(lazydict.Extension):
	...
	def __call__(self, _raw_sacct_text):
		#the final values (initialize to None)
		d = dict.fromkeys(self.target)

		try:
			for line in _raw_sacct_text.split('\n'):
				line = line.strip()
				if line=='':
					continue
				...
				dstep = dict(zip((x[0] for x in keys_sacct),line.split('|')))
...
class x_sacct_SacctReport(lazydict.Extension):
	source= ('JobID',)
	target = ('SacctReport',)
	def __call__(self, JobID):
